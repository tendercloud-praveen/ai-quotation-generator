...
import { getCustomersApi } from "../../services/customerService";
...
export default function InquiriesPage() {
  ...
  const runAi = async (inquiryText = form.text) => {
    ...
    try {
      ...
      const result = await extractInquiryTextApi(text, file);
  ...
  const processInquiry = async () => {
    ...
    if (!form.text.trim() && !file) {
      newErrors.text = "Enter inquiry text or upload a file";
    }
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              className={`w-full rounded-xl border bg-white px-3 py-3 text-sm text-slate-700 outline-none transition dark:bg-slate-900 dark:text-slate-200 ${
                errors.customerId
