...
import { Input } from "../../components/Field";
import { useToast } from "../../components/Toast";
import { registerUser, loginUser } from "../../services/documentservice";
import { setSession } from "../../lib/auth";
import {
  validateEmail,
  validateMobile,
...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    if (!form.password) e.password = "Password is required";
      ...
      e.password = "Use 8+ chars with upper, lower, number & symbol";
    if (form.confirm !== form.password) e.confirm = "Passwords do not match";
    setErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      setSession(loginRes.access_token, {
        id: loginRes.id,
        fullName: loginRes.full_name,
        email: loginRes.email,
        role: loginRes.role.toLowerCase(),
        companyId: loginRes.company_id,
        companyName: form.companyName,
      });

      toast.success(registerRes.message || "User Registered Successfully");
